...
- Refocused the Red Hat combinations matrix on exactly three visible columns: ansible-core, control-node Python, and managed-host Python. Execution-environment image and base-OS details are deferred to the future ansible-navigator demonstration.
- Extended the Red Hat section through 2.11–2.20 while preserving the source boundary. Versions explicitly present in Red Hat's current table are labeled Red Hat-listed; 2.11–2.14 and 2.19 are labeled historical upstream ranges because Red Hat's current table does not list them as supported combinations. This avoids converting historical compatibility into an unsupported product-support claim.
